...
def main(m):
	...
	true_value = linecache.getline('calculated_data.txt',m)
	calculated_value = linecache.getline('calculated_data.txt', n)


	temp = true_value.split()
	(r1, p1, y1) = q2e(float(temp[0]), float(temp[1]), float(temp[2]), float(temp[3]))
	temp = calculated_value.split()
	(r2, p2, y2) = q2e(float(temp[0]), float(temp[1]), float(temp[2]), float(temp[3]))
	...
	reader1.SetFileName('/home/dxt/STL/probe.STL')
	reader2 = vtk.vtkSTLReader()
	reader2.SetFileName('/home/dxt/STL/probe.STL')

	transform = vtk.vtkTransform()
	transform.Translate(model_offset_1, model_offset_2,0)
